...
```{r}
tibble(prob = seq(0, 1, length = 100)) %>%
  mutate (a_seller = dbeta(prob, 91,11),
          b_seller = dbeta(prob, 3,1)) %>%
  pivot_longer(cols=a_seller:b_seller, names_to = "seller") %>%
  ggplot(aes(prob, value, color= seller)) +
  geom_point()
